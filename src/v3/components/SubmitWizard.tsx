...
/**
 * Which circumstances exist is the VARIANT's answer, not this file's. Hong
 * Kong pays nothing for a flower replacement — RULING HK15, 「補花後不算」 — so
 * it is absent from that plugin's `flags` and is never offered here.
 */
const scores = (variant: VariantId, f: WinFlag): boolean =>
  VARIANTS[variant].flags.includes(f)
      ...
      <div class="sheet" role="dialog" aria-modal="true" aria-label={t('wizard.title')}
        ...
        {step === 1 ? (
          <>
            ...
            {/* A rail, not a seven-column grid: at 360px that computes to
                40px, under the 44px floor, and a second row would push this
